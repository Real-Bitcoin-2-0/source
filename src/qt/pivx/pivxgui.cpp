...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("rBTC20 client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("rBTC20"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("rBTC20"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
